...
def euler(dt:float, T:int) -> float:
    """ Performs a simplecticc-euler integration of the non-linear simple pendulum
    and plots various graphs given a time-step (dt) and a max duration (T)
    """
    # Initial Condition
    dt = dt
    omega0 = 5
...
def period_dependance(n:int) -> None:
    ...
    ax.axhline(2 * np.pi * np.sqrt(L / g), color='red', linestyle='--', label='small angle approximation\n$T=2\\pi\\sqrt{\\frac{L}{g}}$')
    ax.set(title='Period T vs Initial Angle $(\\theta)$',
           xlabel='$\\theta$ [deg]',
           ylabel='T [sec]')
    ax.legend()
    #fig.savefig('PeriodDependence.png')
    fig.show()
